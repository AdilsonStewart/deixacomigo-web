exports.handler = async (event) => {
  ...
  // TROQUE SÓ AQUI PELA SUA CHAVE SANDBOX DO ASAAS (copie com o botão)
  ...
  try {
    ...
    const c = await fetch("https://api.asaas.com/v3/customers", {
      ...
      body: JSON.stringify({ name: "Cliente Pix", cpfCnpj: "24994055093", mobilePhone: "47999999999" })
    }).then(r => r.json());
    ...
    return {
      ...
      body: JSON.stringify({
        success: true,
        qrCodeUrl: qr.qrCodeUrl,      // <-- agora manda
        copiaECola: qr.payload,      // <-- agora manda
        paymentId: p.id
      })
    };
  } catch (e) {
